...
public class DataSeeder {
    @Bean CommandLineRunner seed(CardRepository cards,AccountRepository accounts,TripRepository trips,
            TransactionRepository txs,FraudAlertRepository alerts,SupportCaseRepository cases,CustomerRepository customers){
        return args->{
            customers.save(Customer.builder().id("customer-001").displayName("Jessie Han").build());
            customers.save(Customer.builder().id("customer-002").displayName("Milly Li").build());
            if(cards.count()>0){
                ensureVirtualTravelCard(cards);
                configureCardCurrencies(cards);
                curateFutureTrips(trips);
                seedTravelHistory(trips);curateDemoData(trips,txs,alerts,cases);seedLocationMismatchAlert(alerts);seedHighValueConfirmationAlert(alerts);return;
            }
            ...
            configureCardCurrencies(cards);
            trips.save(trip("trip-tokyo","customer-001","Japan","Tokyo",LocalDate.of(2026,10,1),LocalDate.of(2026,10,5),new BigDecimal("2500"),"USD","card-002",Enums.TripStatus.PLANNED));
            trips.save(trip("trip-paris","customer-001","France","Paris",LocalDate.of(2026,11,5),LocalDate.of(2026,11,12),new BigDecimal("3200"),"USD","card-001",Enums.TripStatus.PLANNED));
            ...
            trips.save(trip("trip-expired-card","customer-001","France","Nice",LocalDate.of(2026,12,2),LocalDate.of(2026,12,12),new BigDecimal("1900"),"USD","card-expiring",Enums.TripStatus.PLANNED));
            ...
            saveTx(txs,"txn-hotel","trip-tokyo","Tokyo Hotel","Japan","HOTEL","420.00",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-01T13:30:00Z"),Enums.TransactionType.PURCHASE);
            saveTx(txs,"txn-dining","trip-tokyo","Sushi House","Japan","DINING","82.50",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-02T11:30:00Z"),Enums.TransactionType.PURCHASE);
            saveTx(txs,"txn-atm","trip-tokyo","Shibuya ATM","Japan","CASH","150.00",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-03T13:30:00Z"),Enums.TransactionType.ATM_WITHDRAWAL);
            saveTx(txs,"txn-declined","trip-tokyo","Tokyo Electronics","Japan","SHOPPING","1800.00",Enums.TransactionStatus.DECLINED,"LIMIT_EXCEEDED",Instant.parse("2026-10-04T09:30:00Z"),Enums.TransactionType.PURCHASE);
            ...
            saveTx(txs,"txn-country-mismatch","trip-tokyo","Paris Corner Shop","France","RETAIL","38.50",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-04T12:00:00Z"),Enums.TransactionType.PURCHASE);
            saveTx(txs,"txn-high-value-confirm","trip-tokyo","Tokyo Fine Jewellery","Japan","LUXURY","1420.00",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-04T14:30:00Z"),Enums.TransactionType.PURCHASE);
            ...
            curateDemoData(trips,txs,alerts,cases);seedLocationMismatchAlert(alerts);seedHighValueConfirmationAlert(alerts);
        };
    }
    ...
    private void curateFutureTrips(TripRepository trips){
        Set<String> keep=Set.of("trip-paris","trip-expired-card","trip-reykjavik-demo");
        trips.findAll().stream()
                .filter(t->t.getStatus()!=Enums.TripStatus.COMPLETED&&!keep.contains(t.getId()))
                .forEach(trips::delete);
        trips.save(trip("trip-paris","customer-001","France","Paris",LocalDate.of(2026,11,5),LocalDate.of(2026,11,12),new BigDecimal("3200"),"USD","card-001",Enums.TripStatus.PLANNED));
        trips.save(trip("trip-expired-card","customer-001","France","Nice",LocalDate.of(2026,12,2),LocalDate.of(2026,12,12),new BigDecimal("1900"),"USD","card-expiring",Enums.TripStatus.PLANNED));
        trips.save(trip("trip-reykjavik-demo","customer-001","Iceland","Reykjavik",LocalDate.of(2027,1,8),LocalDate.of(2027,1,14),new BigDecimal("2700"),"USD","card-001",Enums.TripStatus.PLANNED));
    }
    ...
    private void saveTx(TransactionRepository r,String id,String trip,String merchant,String country,String cat,String amount,Enums.TransactionStatus status,String failure,Instant time,Enums.TransactionType type){
        BigDecimal a=new BigDecimal(amount);r.save(TravelTransaction.builder().transactionId(id).customerId("customer-001").tripId(trip).cardId("card-002").merchantName(merchant).merchantCountry(country).merchantCity("France".equals(country)?"Paris":"Tokyo").merchantCategory(cat).originalAmount(a).originalCurrency("USD").billingAmount(a).billingCurrency("USD").exchangeRate(BigDecimal.ONE).transactionTime(time).transactionType(type).status(status).failureCode(failure)
    ...
    private void curateDemoData(TripRepository trips,TransactionRepository txs,FraudAlertRepository alerts,SupportCaseRepository cases){
        ...
        txs.deleteAllById(List.of("txn-decline-2","txn-overseas-off","txn-online-off","txn-atm-limit",
                ...
                "txn-duplicate-b","txn-outside-date","txn-network-fail","txn-insufficient","txn-fraud-block"));
        alerts.findById("alert-review-pending").ifPresent(alerts::delete);
        alerts.findById("alert-open").ifPresent(alerts::delete);
        alerts.findById("alert-safe").ifPresent(alerts::delete);
        cases.findById("CASE-TRV-2048").ifPresent(cases::delete);
    ...
    private void seedCuratedTransactions(TransactionRepository txs){
        ...
        saveTx(txs,"txn-dining","trip-tokyo","Sushi House","Japan","DINING","82.50",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-02T11:30:00Z"),Enums.TransactionType.PURCHASE);
        saveTx(txs,"txn-atm","trip-tokyo","Shibuya ATM","Japan","CASH","150.00",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-03T13:30:00Z"),Enums.TransactionType.ATM_WITHDRAWAL);
        saveTx(txs,"txn-duplicate-a","trip-tokyo","Ginza Department Store","Japan","SHOPPING","210.00",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-03T16:00:00Z"),Enums.TransactionType.PURCHASE);
        saveTx(txs,"txn-country-mismatch","trip-tokyo","Paris Corner Shop","France","RETAIL","38.50",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-04T12:00:00Z"),Enums.TransactionType.PURCHASE);
        saveTx(txs,"txn-high-value-confirm","trip-tokyo","Tokyo Fine Jewellery","Japan","LUXURY","1420.00",Enums.TransactionStatus.APPROVED,null,Instant.parse("2026-10-04T14:30:00Z"),Enums.TransactionType.PURCHASE);
        saveTx(txs,"txn-declined","trip-tokyo","Tokyo Electronics","Japan","SHOPPING","1800.00",Enums.TransactionStatus.DECLINED,"LIMIT_EXCEEDED",Instant.parse("2026-10-04T09:30:00Z"),Enums.TransactionType.PURCHASE);
    ...
    private void seedLocationMismatchAlert(FraudAlertRepository alerts){
        alerts.findById("alert-reported").ifPresent(alerts::delete);
        FraudAlert alert=alerts.findById("alert-location-mismatch").orElseGet(()->FraudAlert.builder()
                .id("alert-location-mismatch").customerId("customer-001").tripId("trip-tokyo")
                .transactionId("txn-country-mismatch").cardId("card-002").build());
        alert.setRiskScore(86);
        alert.setRiskLevel(Enums.RiskLevel.HIGH);
        alert.setDecision(Enums.FraudDecision.REQUIRE_CONFIRMATION);
        alert.setReasonCodes("OUTSIDE_DESTINATION,NEW_MERCHANT_CATEGORY,CUSTOMER_PROFILE_ANOMALY");
        alert.setCustomerMessage("A small purchase in Paris appeared during your registered Tokyo trip. Please confirm whether it was you.");
        alert.setStatus(Enums.AlertStatus.OPEN);
        alert.setCreatedAt(Instant.now().plusMillis(2));
        alert.setResolvedAt(null);
        alert.setCustomerResponse(Enums.CustomerResponse.NONE);
        alerts.save(alert);
    }
    private void seedHighValueConfirmationAlert(FraudAlertRepository alerts){
        FraudAlert alert=alerts.findById("alert-high-value-confirm").orElseGet(()->FraudAlert.builder()
                .id("alert-high-value-confirm").customerId("customer-001").tripId("trip-tokyo")
                .transactionId("txn-high-value-confirm").cardId("card-002").build());
        alert.setRiskScore(72);
        alert.setRiskLevel(Enums.RiskLevel.HIGH);
        alert.setDecision(Enums.FraudDecision.REQUIRE_CONFIRMATION);
        alert.setReasonCodes("HIGH_VALUE_TRANSACTION,NEW_MERCHANT_CATEGORY");
        alert.setCustomerMessage("This high-value jewellery purchase is in Tokyo. Please confirm whether it was you.");
        alert.setStatus(Enums.AlertStatus.OPEN);
        alert.setCreatedAt(Instant.now().plusMillis(1));
        alert.setResolvedAt(null);
        alert.setCustomerResponse(Enums.CustomerResponse.NONE);
        alerts.save(alert);
    }
