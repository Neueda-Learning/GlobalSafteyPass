...
public class AlertService {
    ...
    @Transactional public AlertResponse confirm(String c,String id){FraudAlert a=owned(c,id);a.setStatus(Enums.AlertStatus.CONFIRMED_SAFE);a.setCustomerResponse(Enums.CustomerResponse.CONFIRM);a.setResolvedAt(Instant.now());alerts.save(a);
        txs.findById(a.getTransactionId()).ifPresent(t->{if(t.getStatus()==Enums.TransactionStatus.DECLINED){t.setRecoveryStatus(Enums.RecoveryStatus.SECURITY_CLEARED_RETRY_REQUIRED);t.setRecoveryUpdatedAt(Instant.now());txs.save(t);}});
        audit.log(c,"TRANSACTION_CONFIRMED","ALERT",id,"Safe");return map(a);}
